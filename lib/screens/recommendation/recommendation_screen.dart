...
/// Konten tab Rekomendasi — tanpa bottomNav sendiri.
...
class _RecommendationContentState extends State<RecommendationContent> {
  ...
  decoration: const BoxDecoration(
    color: AppColors.mainBlue,
          ...
          // --- TAB FILTER ---
          SingleChildScrollView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(vertical: 20, horizontal: 20),
            child: Row(children: [_buildFilterChip("Semua", "10", Icons.star), _buildFilterChip("Aktivitas", "2", Icons.directions_run), _buildFilterChip("Nutrisi", "3", Icons.restaurant), _buildFilterChip("Tidur", "2", Icons.bedtime), _buildFilterChip("Terapi", "1", Icons.medical_services), _buildFilterChip("Edukasi", "2", Icons.auto_awesome)])),
          // --- LIST KONTEN ---
          Expanded(child: ListView(padding: const EdgeInsets.symmetric(horizontal: 20), children: [
            if (activeTab == "Semua" || activeTab == "Aktivitas") ...[_buildTaskCard("Aktivitas", "Prioritas Tinggi", "Jalan Kaki 30 Menit Setelah Makan", Icons.directions_walk, Colors.green), _buildTaskCard("Aktivitas", "", "Latihan Kekuatan 2x Seminggu", Icons.fitness_center, Colors.teal)],
            if (activeTab == "Semua" || activeTab == "Nutrisi") ...[_buildTaskCard("Nutrisi", "Prioritas Tinggi", "Metode Piring Harvard (Plate Method)", Icons.restaurant, Colors.orange), _buildTaskCard("Nutrisi", "Prioritas Tinggi", "Eliminasi Minuman Manis & Ultra-Processed Food", Icons.block, Colors.redAccent), _buildTaskCard("Nutrisi", "", "Pilih Karbohidrat Indeks Glikemik Rendah", Icons.grain, Colors.amber)],
            if (activeTab == "Semua" || activeTab == "Tidur") ...[_buildTaskCard("Tidur", "", "Konsisten Jadwal Tidur 7-8 jam", Icons.bedtime, Colors.indigo), _buildTaskCard("Tidur", "", "Hindari Layar 1 Jam Sebelum Tidur", Icons.phonelink_off, Colors.blueGrey)],
  ...
  Widget _buildFilterChip(String label, String count, IconData icon) {
    ...
    return GestureDetector(onTap: () => setState(() => activeTab = label),
      child: Container(margin: const EdgeInsets.only(right: 10), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
        decoration: BoxDecoration(color: isSelected ? AppColors.mainBlue : Colors.white, borderRadius: BorderRadius.circular(25), boxShadow: [if (isSelected) BoxShadow(color: Colors.blue.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))], border: Border.all(color: isSelected ? Colors.transparent : Colors.grey.withOpacity(0.2))),
        child: Row(children: [Icon(icon, size: 16, color: isSelected ? Colors.white : Colors.orange), const SizedBox(width: 8), Text(label, style: TextStyle(color: isSelected ? Colors.white : Colors.black87, fontWeight: FontWeight.bold, fontSize: 13)), const SizedBox(width: 6), Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: isSelected ? Colors.white24 : Colors.grey.shade100, shape: BoxShape.circle), child: Text(count, style: TextStyle(color: isSelected ? Colors.white : Colors.grey, fontSize: 10)))])));
  }

  Widget _buildTaskCard(String category, String priority, String title, IconData icon, Color iconColor) {
    return Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 10, offset: const Offset(0, 4))]),
      child: Row(children: [
        Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: iconColor.withOpacity(0.08), borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: iconColor, size: 22)),
        const SizedBox(width: 15),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Text(category, style: TextStyle(color: iconColor, fontWeight: FontWeight.bold, fontSize: 11)), if (priority.isNotEmpty) ...[const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(4)), child: Text(priority, style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 9)))]]),
          const SizedBox(height: 4), Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.black87))])),
